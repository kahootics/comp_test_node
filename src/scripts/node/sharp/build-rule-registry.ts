...
export async function buildRuleRegistry(rulesDir: string) {
    ...
    for (const { path, module } of allRuleClassModules) {
        ...
        // Validation of module export
        const missing = `Rule module at ${path} is expected to export a rule class, but `;
        const ruleClassesExportsA = exportEntries
            .map(([, v]) => v)
            // verifies it is a function
            .filter(v => typeof v === 'function');
        if (ruleClassesExportsA.length < 1)
            throw new ValidationError(missing +
                "it doesn't export any function"
            );

        // verifies it is a descendant of `Rule`
        const ruleClassesExportsB = ruleClassesExportsA
            .filter(v => extendsClass(v, Rule));
        if (ruleClassesExportsB.length < 1)
            throw new ValidationError(missing +
                "it doesn't export any descendant of the class \"Rule\""
            );

        // check if static properties are actually implemented
        const ruleClassesExports = ruleClassesExportsB
            .map(v => ruleConstructorStaticShape.parse(v));

        if (ruleClassesExports.length > 1)
            throw new ValidationError(
                `Rule module at ${path} exports more than one rule class`
            );
