...
import type { dbDataShape, dbDerivedShape, dbRecordEditables, dbType } from "../db/data-base-types.js";
...
export class RowsBuilder<T extends dbType> {
     ...
     * @param unmodifiabledataShape - Zod schema for the unmodifiable section of each record.
    ...
    constructor(
        dbType: T, 
        unmodifiabledataShape: dbDataShape<T>, 
        deriveddataShape: dbDerivedShape<T>, 
        ...
        const { primitive, nested } = this.#unpackUnmodifiables(unmodifiabledataShape);
        this.#primitiveColumns = primitive;
        this.#nestedColumn = nested;

        // Build the derived columns
        this.#derivedColumns = this.#buildPrimitives(deriveddataShape);
    ...
    #buildPrimitives(shape: Record<string, z.ZodType>): PrimitiveUColumnDescriptor[] {
        return _unpackdataShape(shape).map(c => {
     ...
     */
    #unpackUnmodifiables(unmodifiabledataShape: Record<string, z.ZodType>) {
        const unmodifiables = _unpackdataShape(unmodifiabledataShape);
