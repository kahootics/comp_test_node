...
import { _unwrap, Admitted, type AdmittedType } from "./admitted-types.js";
 ...
 */
export function _unpackdataShape(obj: { [key: string]: z.ZodType; }) {
    return _unpackdataShapeInner(obj);
}

/**
 * @see {@link _unpackdataShape} for full documentation.
 ...
 */
function _unpackdataShapeInner(obj: { [key: string]: z.ZodType; }, prefix?: string, path?: string[]) {
    path ??= [];
    const result: UnmodifiableColumnDescriptor[] = [];

    for (const [key, schema] of Object.entries(obj)) {
        let type: AdmittedType;
        try {
            type = _getAdmittedType(schema);
        } catch (e: unknown) {
            if (e instanceof Error) {
                const label = prefix ? (prefix + '_' + key) : key;
                e.cause = `schema in prop ${label} of object to unpack`;
            } throw e
        }
        ...
        switch (type) {
            case Admitted.OBJECT: {
                ...
                if (unwrapped instanceof z.ZodObject) {
                    result.push(..._unpackdataShapeInner(unwrapped.shape, label, thisPath));
                    break;
                }
                ...
                if (unwrapped instanceof z.ZodArray) {
                    ...
                    result.push(
                        new NestableUColumnDescriptor(
                            ...
                            _unpackdataShapeInner(childrensShape, dLabel)
                        )
                    );
                    break;
                }
